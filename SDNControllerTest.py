...
class Graph(object):
    def __init__(self):
        ...
        self.distances = {}
        self.nbors_list = {}
        self.new_topology = list(settings.topology)
        self.create_graph()
    ...
    def update_graph(self, active_list, deadlinks):
        ...
        from_switches = sorted(set([row[0] for row in settings.topology]))
        to_switches = sorted(set([row[1] for row in settings.topology]))
        all_switches = sorted(list(set(from_switches) | set(to_switches)))
        ...
        numrows = len(settings.topology)
        count = 0
        if not active_list:
            del self.new_topology[:]
        while count < numrows:
            if ((set(settings.topology[count][0]) <= set(off_switches)) or (set(settings.topology[count][1]) <= set(off_switches))):
                count = count + 1
            else:
                self.new_topology.append(settings.topology[count])
                count = count + 1
