...
def calculate_next_hop(source, destination):
    ...
    count = 0
    while count < numrows:
        graph.add_edge(topology[count][0], topology[count][1], int(topology[count][3]))
        count = count + 1
    if source != destination:
        computation = (shortest_path(graph, source, destination))
        return computation[1][1]
    else:
        return destination

if __name__ == '__main__':
    response = calculate_next_hop('6', '5')
    print (" ")
